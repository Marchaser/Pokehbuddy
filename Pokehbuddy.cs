...
namespace Pokehbuddyplug
{
    public partial class Pokehbuddy : HBPlugin
    {
        ...
        public override void Pulse()
        {
			...
			// repair if need, by Marchaser 10/19/2014
		 	if (Styx.StyxWoW.Me.DurabilityPercent <= 0.2) { 
			  new PrioritySelector(
				  new Levelbot.Actions.General.ActionMountVendor()
				  );
			}
                ...
                if (!InPetCombat() && WildBattleTarget() != null)
                {
                    ...
                    if ((3 - dumdum3) >= Pokehbuddy.MySettings.MinPetsAlive)
                    {
                                ...
                                if (stucktimer.ElapsedMilliseconds > 1000 && oldloc.Distance(Styx.StyxWoW.Me.Location) < 0.5)
                                {
                                    if (!Styx.StyxWoW.Me.Combat)
                                    {
                                        ...
                                        if (blacklistcounter > Pokehbuddy.MySettings.BlacklistCounterLimit)
                                        {
                                            ...
                                            Blacklist.Add(WildBattleTarget().Guid, BlacklistFlags.Node, TimeSpan.FromMinutes(1));
                                            blacklistcounter = 0;
                                        }
                                    }
                                }
                        ...
                        if (interacttimer.ElapsedMilliseconds > 1000 && WildBattleTarget().Distance < 12)
                        {
                            ...
                            if (Styx.StyxWoW.Me.Mounted) Styx.CommonBot.Mount.Dismount("Attempting Dismount");
							WoWAuraCollection all_auras = Styx.StyxWoW.Me.GetAllAuras();
							for (int i=0; i<all_auras.Count; ++i) {
								WoWAura current_aura = all_auras[i];
								if (current_aura.Name == "Travel Form")
									current_aura.TryCancelAura();
							}
 ...
 //  e.Cancel = true
        ...
        public static WoWUnit WildBattleTarget()
        {
            ...
            WoWUnit ret = (from unit in ObjectManager.GetObjectsOfType<WoWUnit>(true, true)
                           ...
                           where !Blacklist.Contains(unit.Guid, BlacklistFlags.Node)
            ...
            if (ret != null)
            {
                ...
                else
                {
                    if (dumlevel > 0)
                    {
                        ...
                        Blacklist.Add(ret.Guid, BlacklistFlags.Node, TimeSpan.FromMinutes(1));
                    }
                }
